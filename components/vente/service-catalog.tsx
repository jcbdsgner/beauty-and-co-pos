...
import { formatFcfa, type Service } from "@/lib/data/vente";
...
/** Filtered grid of services/products — the catalogue half of the sale screen for whichever
 * category chip is active (category switching itself lives in the chip rail above, not here). */
export function ServiceCatalog({ services, search, activeFilter, onFilterChange, onAdd, hideFilters }: ServiceCatalogProps) {
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {filtered.map((service) => (
            ...
            >
              <span className="line-clamp-2 pr-8 text-[15px] font-semibold text-[var(--color-gray-900)]">{service.name}</span>
