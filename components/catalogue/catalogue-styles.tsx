    ...
    <div className="flex flex-col gap-6">
      ...
      >
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-px bg-[var(--board-groove)] xl:grid-cols-4">
                ...
                >
                  <div className="relative flex aspect-[4/3] items-center justify-center bg-[var(--brand-rose-soft)]">
                    <span className="absolute left-2 top-2">
                      <Legend className="text-[var(--brand-taupe-muted)]/60">Pl. {String(i + 1).padStart(2, "0")}</Legend>
                    </span>
                    <Icon aria-hidden className="size-9 text-[var(--brand-taupe-muted)]" />
                    {style.trending && (
                      <span className="absolute bottom-2 right-2 rounded-[6px] bg-[var(--board-amber)] px-1.5 py-0.5 font-[family-name:var(--font-heading)] text-[0.55rem] font-bold uppercase tracking-[0.1em] text-white">
                        Tendance
                      </span>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col gap-0.5 border-t border-[var(--board-groove)] p-3">
                    <p className="line-clamp-2 text-sm font-semibold text-[var(--color-gray-900)]">{style.name}</p>
                    <p className={cn("mt-auto pt-1 text-sm font-semibold tabular-nums text-[var(--button-2-color)]")}>
