...
Landing, refonte Figma 156-72 (base daisyUI, cartes — plus de `Board`/`Lane`). `BoardHeader` (titre « {nom}, {salon du poste} » — salon porté par la session, pas par la personne ; plus de sous-titre ; `action` : filtre de salon `ChipFilter` **Tous les salons** — défaut · Almadies · Sea Plaza (ADR 0028, étendu à l'Accueil) ; **« Créer un rendez-vous »**, ouvre [`PriseRdvModal`](../components/prise-rdv/prise-rdv-modal.tsx) — le parcours de prise de rendez-vous b&co recopié, ADR 0032). Le bloc de compteurs « Le point du jour » est **retiré** (la journée est visible, la file a son lien). Une bande puis deux sections :
0. **Réservations reçues** — [`AccueilUnseenReservations`](../components/journee/accueil-unseen-reservations.tsx) : bande rose tout en haut, présente seulement s'il y a des réservations en ligne **Non vues** (ADR 0030, rév. 25/09) — tous jours confondus, hors filtres de la section Rendez-vous, la plus récemment reçue en tête (« Reçue il y a N min » · jour + heure · payeuse · composition · prestations · praticiennes · salon). Ligne → fiche réservation (la marque vue) ; « Tout marquer comme vu » ; 4 lignes puis « Afficher les N autres ». Remplace le point sur l'avatar de la carte du jour.
...
En tête : **recherche** (acheteuse, destinataire, n° de carte, prestation, téléphone — accents/casse/tirets ignorés) + bouton **Scanner une carte** (`highlight-rose`, dialogue `ScanCamera` + saisie → remplit la recherche). Sans recherche : « À imprimer · N » (`a_imprimer`) puis « Prêtes à remettre · N » (`imprimee`). Avec recherche : + « Remises / expédiées · N » (`remise`/`livree`, lecture seule, « Remise le JJ/MM » via `handedOverAt`). Code exact → tuile mise en évidence (ombre rosée) ; après un scan, focus sur son action. Tuile [`GiftCardTile`](../components/journee/gift-card-tile.tsx) (partagée avec l'Accueil) : badge neutre Livraison/Retrait (icône) + code · acheteuse · **contenu** (`giftCardContent` : montant, ou noms des prestations d'une carte `prestations`) · cible (« Livrer à … » / tél `formatPhone` / adresse complète, ou « Retrait au comptoir » / « Prévenir au … ») · « Commandée il y a N j » (plus de bord ambre). Pied : **Imprimer** (`useReactToPrint` → `printGiftCardOrder`) puis **Réimprimer** (icône) + **Marquer comme remise/expédiée** (`markGiftCardOrderHandedOver`, pose `handedOverAt`). Données : [`lib/data/cartes-cadeaux.ts`](../lib/data/cartes-cadeaux.ts) (`GIFT_CARD_ORDERS`, `carteCadeauByCode`, `normalizeGiftCardCode`, `giftCardContent`). Impression : code-barres + code cadeau juste en dessous ([`Barcode`](../components/shared/barcode.tsx), hors écran), plus de carte entière.
...
| Dialogue « Un pourboire ? » | `TipDialog` | [`components/comptoir/tip-dialog.tsx`](../components/comptoir/tip-dialog.tsx) — ouvert par « Encaisser » (panier), avant le Règlement : 9 montants (2k→30k) + « Autre » (clavier), « Sans pourboire » ; × reste sur le panier. Le Règlement encaisse vente + pourboire (`pendingTip`), réservé sur le moyen de la dernière part (ADR 0034) |
...
- **`atoms/`** — `button`, `badge`, `avatar`, `card`, `text-input`, `textarea`, `select`, `checkbox`, `switch`, `search-input`, `icon-button`, `logo`, `hero-number`, `progress-bar`, `round-step-button`, `photo-placeholder`, `spinner`, `skeleton`, `separator`, `tooltip`, `field-label`, `icons`.
...
- Partagé : [`components/shared/client-search-field.tsx`](../components/shared/client-search-field.tsx) (recherche cliente Comptoir + Messages) ; [`components/shared/barcode.tsx`](../components/shared/barcode.tsx) (`Barcode` — code-barres démo, reçu imprimé + carte cadeau imprimée).
