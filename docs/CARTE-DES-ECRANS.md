...
File des `GiftCardOrder` non résolus (`a_imprimer` + `imprimee`). Une ligne = acheteur + montant + badge Retrait/Livraison + jeton statut ; coordonnées bénéficiaire sous les lignes Livraison. Actions : **Imprimer** (`useReactToPrint` sur un `GiftCard` hors-écran par row → `printGiftCardOrder`) puis **Marquer comme remise/expédiée** (`markGiftCardOrderHandedOver` → quitte la file). Store : `giftCardOrders` slice. Données : [`lib/data/cartes-cadeaux.ts`](../lib/data/cartes-cadeaux.ts) (`GIFT_CARD_ORDERS` + ledger `CARTES_CADEAUX` inchangé). Face imprimée : [`components/shared/gift-card.tsx`](../components/shared/gift-card.tsx) (jamais rendue à l'écran).
