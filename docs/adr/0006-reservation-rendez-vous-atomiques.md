...
**`Service` gagne `twoPractitionersEligible: boolean`.** Le Menu (`lib/data/menu.ts`) est régénéré **verbatim** depuis le catalogue de réservation b&co (`b&co/lib/data/booking-services.ts`) : 107 prestations, ids/libellés/prix/durées/éligibilité « à 2 » identiques. (Structure des catégories depuis divergée : le Menu regroupe Mini&Co en une seule catégorie à deux sous-catégories Hair / Spa là où b&co la scinde en deux — cf. `lib/data/menu.ts`.) La pastille « 2 » sur la tuile du Comptoir signale l'éligibilité.
