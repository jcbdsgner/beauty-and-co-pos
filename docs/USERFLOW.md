...
| **Praticienne** | Un membre de l'équipe. `role` ∈ coiffeuse · esthéticienne · **ménage** · accueil (libellés affichés au masculin — la fonction, pas la personne). Porte un **horaire hebdomadaire récurrent** (`weeklySchedule` : pour chaque jour de semaine, une plage de présence ou repos — ADR 0020) affiché au Planning. Le ménage et l'accueil ne tiennent jamais de rendez-vous ; le ménage figure quand même au Planning, l'accueil non (v2.4). | `1,1 —— 0,N Rendez-vous` (assignée) · `1,1 —— 0,N Rendez-vous` (seconde, prestations « à 2 ») | `présente / absente aujourd'hui / repos` (dérivé, pas un cycle de vie stocké) |
...
Identifier la cliente (dialogue unique, caméra réelle — atteint du bouton « Scanner » du ticket)
- cadre de visée, erreur caméra affichée si besoin
- sous le viseur, **un seul champ de code** pour un code **carte de fidélité** → attache la fiche via `loyaltyCode`. Une fois la fiche attachée, **sa carte cadeau active se lie d'elle-même** à la vente (ADR 0013) — la carte cadeau ne se scanne ni ne se saisit nulle part.
- la **caméra lit les QR toute seule** (`BarcodeDetector`, Chromium) ; le QR lu est routé comme un code saisi. Pas de bouton « simuler » — en prototype, n'importe quel QR vaut la carte de fidélité de démonstration.
- **bouton « Annuler »** (le dialogue ne se ferme pas à l'overlay/Échap)
- jeton **au porteur** : présenter la carte de fidélité suffit, aucune vérification de titulaire, aucun rappel d'identité bloquant au moment de dépenser des points ou un solde cadeau ; garde-fou = nom de la cliente lisible en tête de ticket + le **« Retirer »** de la ligne cliente du ticket (détache la fiche ; la carte cadeau part avec elle) (ADR 0013)
...
- ordre de calcul : déjà payé → remises accordées (par ligne) → points → carte cadeau (clampée à ce qui reste dû) → acompte ; le pied de ticket **ventile** chaque mécanisme, jamais un total « Remises » agrégé
- **Moyens de paiement** : 4 grandes tuiles — **Carte** et **Espèces** en grosses icônes, Wave et Orange Money avec leur logo, **toutes libellées**
- **Paiement en 1 à 3 parts** : une part = tout le montant, rien à saisir. « Payer en plusieurs fois · jusqu'à 3 » ajoute une part ; toutes les parts sauf la dernière se tapent au pavé, **la dernière est toujours « le reste », calculée**. Un même moyen peut revenir (deux cartes) ; Espèces une seule fois. Choisir un moyen passe à la part suivante sans moyen (3 parts = 3 touches)
- Espèces : « Reçu » tapé au pavé → **Rendu** calculé en direct
- à droite du pavé, ou à sa place : ce qu'il faut vérifier (« Passez la carte sur le terminal… », « Vérifiez la réception sur le téléphone du salon »)
- [ Parts qui dépassent le total → « Confirmer » désactivé, message « Les parts dépassent le total de X F » — corriger une part au pavé, jamais tout reprendre ]
- [ Remises + avantages + acompte couvrent tout → « Rien à encaisser », « Confirmer » clôture la vente ]
- **Confirmer l'encaissement** (pied du ticket, à la place exacte d'« Encaisser » au panier) → Reçu ; sous le bouton, la raison tant qu'il est désactivé

Reçu (dans le Comptoir déployé — ADR 0031)
- le ticket, toujours à droite, **devient le reçu imprimable** ; à gauche : « Vente encaissée », le montant, chaque part payée (icône + moyen + montant, reçu / rendu pour les espèces), points gagnés
- **Motif de remise** (si une remise accordée est en jeu) : **carte inline** en tête des actions, signal ambre, texte libre ≥ 3 caractères, **un seul motif pour toutes les remises** de la vente ; les autres actions restent désactivées tant qu'il manque (remplace l'ancienne modale bloquante)
...
**Ce que ce calque résout structurellement** : plus de « page Vente » qu'on quitte et qui perd son état ; un seul endroit pour identifier la cliente (son code de fidélité — la carte cadeau, elle, se lie toute seule).
...
**Décisions actées** : le rendu de monnaie sur les paiements impliquant Espèces est une **capacité nouvelle** ; l'auto-remplissage du panier depuis un rendez-vous est explicite (message dans le panier) ; la carte cadeau devient un instrument prépayé (reliquat conservé, cf. ADR 0002) et **auto-liée à la cliente identifiée** — plus rien à scanner ni saisir pour elle (ADR 0013 rév.) ; la « remise manager » devient une remise réceptionniste bornée à **10 % sans aucun code**, 20 % avec un code manager ponctuel, motif obligatoire (cf. ADR 0003, 0008) ; un reçu imprimable existe enfin ; l'identification de la cliente devient **conditionnelle** (facultative en vente de produits, obligatoire dès qu'une prestation est au panier) via **un seul dialogue** à un champ de code de fidélité (ADR 0013). Le panneau Remise n'est plus un dialog : il se **déroule en pied de panier**, au-dessus du Total. Un **acompte** déjà versé sur la plateforme externe (porté par la réservation) se déduit du Total pour afficher un **Reste à encaisser** — lu tel quel, jamais saisi dans l'app, et distinct des trois mécanismes de Remise (ADR 0015).
...
- Identification de la cliente : « Chercher une cliente » (mécanisme unique, partagé avec Répertoire et Formulaire rendez-vous), « Scanner » (code de fidélité), ou « + Nouvelle cliente ». **Conditionnelle** (ADR 0013) : facultative pour une vente produits / boissons, obligatoire dès qu'une prestation est au panier. Identifier la cliente lie aussi sa carte cadeau éventuelle
...
- Trois onglets **Prestations · Produits · Boissons** du Menu (**défaut Produits**), recherche, rail de catégories et sous-catégories (marques + gammes côté Produits ; rien côté Boissons), grille de tuiles → ajout au panier (incrémente la quantité si la ligne existe déjà)
- Panier : quantité par ligne, retrait de ligne (pas d'assignation de praticienne — retirée)
- Remise : panneau **déroulant** en pied de panier (au-dessus du Total, pas une popup), trois mécanismes cumulables (carte cadeau auto-liée, points fidélité, remise accordée : ≤ 10 % des prestations sans code, ≤ 20 % avec un code manager), erreur inline explicite ; lignes ventilées au pied de ticket
  - carte cadeau : instrument prépayé (montant ou prestations) rattaché à la fiche de sa détentrice — **rien à saisir ni à scanner**, elle se lie dès que la cliente est identifiée (ADR 0013) ; **application ajustable** (combien du montant / quelles prestations), retirable ; reliquat affiché ; carte au porteur / d'un tiers → pas encaissable
...
- Atteint du bouton « Scanner » du ticket
- Cadre de visée ; la caméra lit les QR toute seule (`BarcodeDetector`)
- **Un seul champ de code** pour un code **carte de fidélité** → attache la fiche. Sa carte cadeau active se lie ensuite d'elle-même (ADR 0013). Le QR lu suit le même routage (en prototype, n'importe quel QR vaut la carte de démo)
- **Bouton « Annuler »**
- Jeton **au porteur** : aucune vérification de titulaire, aucun rappel d'identité bloquant au moment de dépenser des points ou un solde cadeau ; garde-fou = nom de la cliente lisible en tête de ticket + « Retirer » sur la ligne cliente pour détacher
...
```
Planning  (un seul écran — refonte totale, ADR 0020 : le programme de chaque praticienne, et
seulement ça ; la liste de réservations à encaisser ne vit plus ici, voir Accueil)
- « Aujourd'hui » (seulement si on a navigué ailleurs) → revient au jour réel
- naviguer d'un jour / d'une semaine à l'autre (pas de vue Mois — délibérément hors périmètre)
...
- une **ligne par praticienne**, le temps défile horizontalement, ses rendez-vous positionnés dedans (vue Jour) ; en vue Semaine, chaque ligne tient ses 7 jours en cellules compactes (horaire + nombre de rdv, ou repos)
- chaque praticienne porte son **horaire hebdomadaire récurrent** (`weeklySchedule`) et son état (présente / absente aujourd'hui / repos) ; la zone hors de son horaire du jour affiché est **grisée** (ligne entière hachurée un jour de repos) ; le **ménage** figure sans jamais tenir de rendez-vous ; l'accueil non. Sidebar de filtre (case à cocher par praticienne) plutôt qu'un rail de colonnes. Par praticienne :
  - « Isoler cette ligne » → ne garder qu'elle ; « Tout afficher » annule
...
**Décisions actées (Planning)** : la gestion de l'agenda est une section propre, Équipe y est fondue (`docs/adr/0005`) ; depuis sa refonte totale (ADR 0020), le Planning est **exclusivement** le calendrier par praticienne (une ligne par praticienne, temps horizontal, zone grisée hors horaire hebdomadaire) — la liste de réservations à encaisser n'y vit plus, elle reste sur l'Accueil ; **la création de réservation reste hors de l'app** (faite en ligne, `docs/adr/0006`) mais l'**ajustement** d'une réservation qui arrive se fait au comptoir — reprogrammer, réassigner, ajouter / retirer, annuler avec motif ; seul blocage dur : chevauchement praticienne (v2.4, `docs/adr/0009`) ; un rendez-vous est actif ou annulé (pas de « en attente / confirmé ») ; « Encaisser » agit au niveau **réservation** (toutes les prestations, un seul règlement) ; annuler conserve un statut Annulé ; l'absence de dernière minute d'une praticienne a un geste dédié ; chaque praticienne porte son horaire hebdomadaire récurrent et le ménage y figure sans jamais tenir de rendez-vous.
...
Profil
- fiche de l'utilisateur au poste : nom, fonction (lecture seule)
...
- **Remise** : trois mécanismes cumulables (points fidélité, carte cadeau *prépayée* avec reliquat — auto-liée à la cliente identifiée, ADR 0013 —, remise accordée ≤ 10 % des prestations sans code, jusqu'à 20 % avec un **code manager** ponctuel, + motif obligatoire après l'encaissement). Cf. ADR 0002–0003, 0008. L'objet **Remise** (`{ mode, valeur }`) est le même que celui d'une Relance de reconquête.
...
- ~~**Authentification réelle du code réceptionniste** pour une remise accordée~~ — **caduc (ADR 0008 v2)** : il n'y a plus de code réceptionniste. Sous 10 %, aucun code ; 10–20 %, un code manager non vérifié (mock). Reste éventuellement ouverte : la vérification réelle du **code manager**.
