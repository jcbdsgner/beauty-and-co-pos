...
- trois onglets **Prestations · Produits · Boissons**, recherche, rail de catégories (Prestations : grandes catégories + familles ; Produits : marques — Kérastase et ses gammes, Saryna Keys, Nefertiti, Autres ; Boissons : sans rail), grille de tuiles → ajout au panier (incrémente si déjà présent) ; **le Menu s'ouvre sur Produits** (la vente à froid est une revente ; l'onglet Prestations reste accessible). Une boisson compte dans le total et les points gagnés, jamais dans l'assiette d'une remise (ADR 0016)
...
- Identification de la cliente : « Chercher une cliente » (mécanisme unique, partagé avec Répertoire et Formulaire rendez-vous), « Scanner », ou « + Nouvelle cliente ». **Conditionnelle** (ADR 0013) : facultative pour une vente produits / boissons, obligatoire dès qu'une prestation est au panier
