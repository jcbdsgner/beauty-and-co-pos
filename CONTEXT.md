...
**Accueil**:
La section d'atterrissage de l'app : le centre de pilotage du jour — la section **« Rendez-vous »** (la **vue journée** « Le jour », ADR 0014/0018 ; depuis ADR 0020, c'est le **seul** endroit de l'app où les réservations du jour se consultent — le Planning ne les affiche plus), cartes cadeaux à imprimer. « Rendez-vous » se bascule entre deux vues (`SegmentedToggle`, ADR 0019) : **Liste** (défaut, grille fixe de 3 cartes, une par réservation, triée par heure, chacune avec « Encaisser ») et **Calendrier** (une colonne verticale par heure, un bloc = une réservation entière positionnée sur son passage, avatars des praticiennes concernées, réservations simultanées posées côte à côte). Chaque carte/bloc porte la **composition** du passage (« 1 femme + 1 enfant », « 2 femmes »…, ADR 0018) plutôt qu'un résumé des prestations ; la carte Liste ne grossit jamais (au-delà de 3 lignes — prestations puis extras, boisson/produit pré-commandé à emporter —, le reste se résume en « + N de plus »), le détail complet vivant dans la fiche réservation. Son en-tête porte aussi un accès **« Créer un rendez-vous »**, qui ouvre le formulaire de création au comptoir (ADR 0027) — utile quand une cliente veut réserver par téléphone. Porté le nom **Journée** pendant la refonte v2 du userflow, revenu à **Accueil**. Ne ressuscite pas le verbe **Accueillir** (retiré — voir Encaisser) : la section s'appelle Accueil, le geste au comptoir reste « Encaisser ». Coexiste avec le rôle « Accueil » de la vue Équipe (la fonction de la personne au comptoir) — même mot, contextes distincts (item de navigation vs filtre de rôle). Reste borné à **aujourd'hui seulement** : aucune navigation de date, contrairement au Planning (ADR 0019).
